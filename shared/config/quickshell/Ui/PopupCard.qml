...
PopupWindow {
  ...
  // A grabbing Wayland popup must disappear as soon as it closes. Keeping it
  // mapped for the opacity tail leaves its grab at the top of Qt's popup stack
  // and causes the next popup to acquire the wrong transient parent.
  visible: triggerMode === "click" ? open : (open || card.opacity > 0)
  // Native popup focus works across wlroots compositors, including Niri.
  // Quickshell sets visible=false when focus is lost or the user clicks
  // outside; onVisibleChanged below mirrors that dismissal into the owner's
  // state so the next click can open the popup normally.
  grabFocus: triggerMode === "click"
  color: "transparent"
  ...
  BorderSurface {
    ...
    Behavior on opacity {
      enabled: root.triggerMode !== "click"
