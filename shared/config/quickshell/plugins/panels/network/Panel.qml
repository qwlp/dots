...
Panel {
  ...
  // Keep the hero identity tied to the same live native state as the icon and
  // network rows so an empty `info` sample cannot label an active link as
  // "No connection".
  readonly property string connectionTitle: {
    if (kind === "wifi")
      return (connectedWifiNetwork && connectedWifiNetwork.name) || info.ssid || "Wi-Fi"
  ...
  Timer {
    ...
    interval: 2800
    running: root.opened && (root.kind === "ethernet" || root.kind === "wifi")
    repeat: true
        ...
        Column {
          ...
          Text {
            ...
            readonly property string title: {
              return root.connectionTitle
            }
            readonly property string detail: root.info.type === root.kind ? root.headerDetail() : ""
          ...
          Text {
            ...
            width: parent.width
            text: {
              if (root.kind === "wifi" || root.kind === "ethernet") return root.connectionPhrase.toUpperCase()
