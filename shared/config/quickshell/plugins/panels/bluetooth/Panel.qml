...
Panel {
  ...
  Timer {
    ...
    onTriggered: {
      ...
      // Clear ownership before the write. Quickshell updates the property
      // asynchronously; retaining the debt would re-arm duplicate stops while
      // BlueZ is still processing the first one.
      root.owesDiscoveryStop = false
