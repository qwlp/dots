...
Panel {
  ...
  Process {
    id: batteryProc
    command: ["sh", "-c", "dev=$(upower -e 2>/dev/null | awk '/battery_BAT/ { print; exit }'); [ -n \"$dev\" ] || exit 0; upower -i \"$dev\" | awk -F: 'function clean(v){gsub(/^[ \\t]+|[ \\t]+$/,\"\",v); return v} /percentage:/{pct=clean($2); print \"percentage\\t\" pct} /energy-full:/{full=clean($2)+0} /energy-full-design:/{design=clean($2)+0} /charge-cycles:/{print \"cycles\\t\" clean($2)} /time to empty:|time to full:/{print \"time\\t\" clean($2)} /energy-rate:/{print \"rate\\t\" clean($2)} END {if(full>0 || design>0) printf \"size\\t%.1f / %.1f Wh\\n\", full, design; if(pct!=\"\") print \"threshold\\t\" pct}'"]
    stdout: StdioCollector { waitForEnd: true; onStreamFinished: root.updateKeyValue(text, "battery") }
  }

  Process {
    id: profilesProc
    command: ["sh", "-c", "powerprofilesctl list 2>/dev/null | awk '/^[ \\t]*\\*?[ \\t]*(performance|balanced|power-saver):/ { active=($1==\"*\"); name=$NF; sub(/:$/,\"\",name); print name \"\\t\" active }'"]
