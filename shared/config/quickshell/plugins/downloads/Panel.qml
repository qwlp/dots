...
Panel {
  ...
  KeyboardPanel {
    ...
    externalDragActive: downloadsPanel.fileDragActive
    ...
    PanelKeyCatcher {
      ...
      Text {
        ...
        text: "↵ open · drag to share · t trash · space folder"
  ...
  component FileRow: CursorSurface {
    ...
    readonly property url dragUrl: fileUrl
    ...
    // A native URI-list drag is understood by file managers, browsers and
    // messaging clients. CopyAction keeps dragging non-destructive: the
    // original download remains in place regardless of the destination.
    Drag.dragType: Drag.Automatic
    Drag.supportedActions: Qt.CopyAction
    Drag.proposedAction: Qt.CopyAction
    Drag.mimeData: ({
      // A typed URL list makes Qt call QMimeData::setUrls(), which is what
      // Telegram, Chromium and file managers query on drop.
      "text/uri-list": [fileRow.dragUrl],
      "x-special/gnome-copied-files": "copy\n" + fileRow.fileUrl + "\n",
      "application/x-kde-cutselection": "0",
      "text/plain": fileRow.file ? String(fileRow.file.path || "") : ""
    })
    ...
    DragHandler {
      ...
      enabled: fileRow.fileUrl !== ""
      dragThreshold: 8
      onActiveChanged: {
        if (active) {
          downloadsPanel.fileDragActive = true
          downloadsPanel.setFileCursor(fileRow.rowIndex)
          fileRow.grabToImage(function(result) {
            if (!rowDrag.active) return
            fileRow.Drag.imageSource = result.url
            fileRow.Drag.hotSpot = Qt.point(rowDrag.centroid.position.x,
                                            rowDrag.centroid.position.y)
            fileRow.Drag.active = true
          })
        } else {
          fileRow.Drag.active = false
          downloadsPanel.fileDragActive = false
        }
      }
    }
