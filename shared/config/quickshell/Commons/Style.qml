...
QtObject {
  ...
  // ---------------------------------------------------------- typography
  //
  // Use SF Pro Text throughout the bar and panels. Qt will still fall back to
  // the installed Nerd Font for glyphs that SF Pro does not contain.
  ...
  property string resolvedFontFamily: "SF Pro Text"
  ...
  function refresh() {
    // Niri does not expose decoration geometry over its IPC. These are shell
    // presentation values now and may be overridden by shell.toml.
    cornerRadius = 8
    gapsOut = 6
  }
  ...
  property Process fcMatchProc: Process {
    id: fcMatchProc
    command: ["fc-match", "-f", "%{family[0]}", root.fontFamily]
