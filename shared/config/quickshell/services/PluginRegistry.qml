...
QtObject {
  ...
  function moveBarEntry(config, id, placement) {
    ...
    // A centering anchor only has meaning inside the center section. Keep the
    // setting consistent with drag/move operations so a stale anchor cannot
    // make a widget appear impossible to move back to the middle.
    var centerAnchor = Util.canonicalWidgetId(String(config.bar.centerAnchor || ""))
    if (key === centerAnchor && target.section !== "center") {
      config.bar.centerAnchor = ""
    } else if (target.section === "center" && centerAnchor === "") {
      config.bar.centerAnchor = key
    }
    return ""
  }
