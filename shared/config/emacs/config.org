...
that should outlive an individual task, while Google Calendar supplies external
...
- The generated Google Calendar file lives under Emacs's state directory, not
  in =~/org/=, but it is included in the agenda.

The agenda reads the inbox, task, project, and Google Calendar files.  The Org
files under =~/org/= are committed, rebased, and pushed if that directory is a
Git repository: asynchronously at startup and once more when Emacs exits.  A
sync failure leaves all files and Git state intact for manual recovery.
...
If =~/.config/org-gcal/config.el= exists (or =ORG_GCAL_CONFIG= names another
file), press =C-c o C= to fetch Google Calendar events.  Set
=ORG_GCAL_AUTO_FETCH= to a nonempty value to also fetch five seconds after
startup and every 30 minutes.  Automatic fetching is opt-in because unlocking
the encrypted OAuth token during startup otherwise causes a passphrase prompt.
Imported events appear in the normal dashboard and agenda.  The optional org-gcal auto-archiver is
disabled because malformed or unsupported event timestamps can otherwise abort
the whole fetch; this does not disable calendar synchronization.

The machine-local config contains credentials and calendar mappings and should
not be placed in this shared literate config.

On a new computer, run =M-x tsp/org-gcal-setup=.  Select the OAuth client JSON
downloaded from Google Cloud and enter the Google Calendar account address.
Emacs installs the credentials and writes the machine-local configuration with
owner-only permissions.  Then press =C-c o C= and complete Google's browser
authorization once.
