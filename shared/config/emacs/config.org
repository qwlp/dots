...
In Org buffers, long lines are filled at 80 columns.  Type =,java= or =,py= to
insert a Java or Python source block immediately.  For other languages, type a
comma followed by the language and =TAB= to insert a source block.
