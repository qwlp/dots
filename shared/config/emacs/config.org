...
In Org buffers, long lines are filled at 80 columns.  Type =,java= to insert a
Java source block immediately.  For other languages, type a comma followed by
the language and =TAB=, such as =,py=, to insert a source block.
The aliases =el=, =py=, and =sh= expand to =emacs-lisp=, =python=, and =shell=;
other language names are used as typed.
Java source blocks are formatted with =clang-format= whenever the Org file is
saved and can be compiled and run with =C-c C-c=; inline images are refreshed
afterward.
